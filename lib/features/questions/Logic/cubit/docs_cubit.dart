...
class DocsCubit extends Cubit<DocsState> {
  ...
  final String _collectionPath='Docs';

  Future<void> loadDocs({String? filetype}) async {
    emit(DocsLoading());
    try {
      ...
      final docs =
          snapshot.docs.map((e) => DocsModel.fromMap(e.data() as Map<String, dynamic>)).toList();
  ...
  Future<void> deleteDoc(DocsModel doc) async {
    emit(DocsLoading());
    try {
